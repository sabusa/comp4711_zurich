...
if (!function_exists('showImage2')) {
    ...
    function showImage2($label, $name, $width = 120, $height = 80) {
        $CI = &get_instance();
        $parms = array(
            'label' => $label,
            'name' => $name,
        ...
        return $CI->parser->parse('_fields/x_submitted2', $parms, true);
    }

}

/**
 *  Construct a file uploader.
 * 
 * @param string $label Descriptive label for the control
 * @param string $name ID and name of the control; s/b the same as the RDB table column
 * @param string $explain Help text for the control
 * @param int $size width in ems of the input control
 */
if (!function_exists('makeFileUploader')) {

    function makeFileUploader($label, $name, $explain, $size = 25) {
        $CI = &get_instance();
        $parms = array(
            'label' => $label,
            'name' => $name,
